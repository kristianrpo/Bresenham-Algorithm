...
/**
 * La clase EventoMouse extiende JPanel y implementa MouseListener para manejar eventos del mouse.
...
public class Bresenham extends JPanel implements MouseListener {
    ...
    public void plotLine(Graphics2D g2d,int x1, int y1, int x2, int y2){
        ...
        int temp = 0; // variable de control para definir cuando se pinto la linea.
        ...
        while ((dx < dy || temp != dx) && (dy < dx || temp != dy)) { // mientras que no se haya recorrido toda la distancia a pintar
            ...
            if (dx >= dy) { //verifico si me estoy moviendo mas en x o en y, si es en x
                ...
                if (P < 0) { // si mi variable de decision es menor a 0, escojo el mismo y
                    P = P + 2 * dy;
    ...
    /**
     * Método principal que crea un JFrame y agrega una instancia de EventoMouse a él.
     */
